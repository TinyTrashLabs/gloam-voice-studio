...
struct VoiceSidebarView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            // Section switch: Studio (speak with reusable voices) vs Create Voice
            // (the Foundry — mint a new voice with qwen3-design). Sidebar stays put.
            Picker("", selection: Binding(
                ...
                set: { newValue in
                    ...
                    if newValue == StudioSection.createVoice.rawValue { model.editingVoiceSlug = nil }
                })) {
                Text("Studio").tag(StudioSection.studio.rawValue)
                Text("Create Voice").tag(StudioSection.createVoice.rawValue)
            }
            .pickerStyle(.segmented)
            .padding(.horizontal, 14)
            .padding(.top, 10)
            .accessibilityIdentifier("section-switch")
